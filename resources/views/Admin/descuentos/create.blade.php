	...
	{!! Form::open(['url'=>'/Descuento', 'method'=>'post', 'autocomplete'=>'off', 'spelcheck'=>'false']) !!}
		...
		@if(Session::has('lp')){{session('lp')}}@endif
			...
			<tr>
				...
				<td>
					<input type="text" name="Desc_Des" placeholder="obligatorio" size="20" maxlength="20" value="{{old('Cli_Nom')}}" required autofocus>
					@if($errors->has('Cli_Nom'))
					<span class="help-block">{{$errors->first('Cli_Nom')}}</span>
					@endif
				</td>
		...
		<table class="sobre">
			<tr><td>
				...
				<tr class="porc_todos"><td>Aplicar a todos: <input type="number" id="porc_tod" max="99" min="0" onkeypress="if(this.value.length==2) return false;"> %</td></tr>	
			</td></tr>	
		</table>
@endsection
